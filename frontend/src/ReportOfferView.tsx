...
import './ReportOfferViewStyles.css';
...
function ReportOfferView() {
    ...
    const validateEmail = (email: string) => {
        const regex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
        return typeof email === 'string' && email.length > 0 && regex.test(email.toLowerCase());
    };
    ...
    const sendReport = (reportObject: { email: any; reportReason: any; offerId: any; }) => {
    ...
    return (
        <>
            ...
            <div className='mainReportView'>
                ...
                <div>
                    Zgłoś ją tutaj. Wprowadź poniżej swój adres email, oraz podaj powód naruszenia zasad.
                </div>
                ...
                <div>
                    <button className='reportButton' onClick={handleSubmit}>Zgłoś</button>
                </div>
            </div>
            <FooterView />
        </>
